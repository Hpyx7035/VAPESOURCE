...
public class WatchdogNoSlow extends Mode<NoSlow> {
    ...
    public final Listener<PacketSendEvent> onPrePacket = event -> {
        ...
        if (packet instanceof C08PacketPlayerBlockPlacement) {
            if (mc.gameSettings.keyBindUseItem.isKeyDown() && (mc.thePlayer.getHeldItem() != null && (mc.thePlayer.getHeldItem().getItem() instanceof ItemFood || mc.thePlayer.getHeldItem().getItem() instanceof ItemBucketMilk || (mc.thePlayer.getHeldItem().getItem() instanceof ItemPotion && !ItemPotion.isSplash(mc.thePlayer.getHeldItem().getMetadata()))))) {
                ...
                final float f = (float) (position.hitVec.xCoord - (double) position.getBlockPos().getX());
                final float f1 = (float) (position.hitVec.yCoord - (double) position.getBlockPos().getY());
                final float f2 = (float) (position.hitVec.zCoord - (double) position.getBlockPos().getZ());
                mc.thePlayer.sendQueue.addToSendQueueUnregistered(new C08PacketPlayerBlockPlacement(position.getBlockPos(), position.sideHit.getIndex(), ((C08PacketPlayerBlockPlacement) packet).getStack(), f, f1, f2));
            }
        }
    };
